...
| `analysis_mode` | string | No | Optional analysis mode to activate the multi-step agent. Supported: `"simple"` (default), `"regulatory_compliance"`, `"esg_financial_analysis"` |
...
- **With `analysis_mode`:**
  - `"simple"` (or omitted): uses the classic single-step RAG+LLM flow.
  - `"regulatory_compliance"`: triggers a multi-step flow (requisitos regulatorios → evidencias del cliente → matriz de cumplimiento).
  - `"esg_financial_analysis"`: triggers a multi-step flow (KPIs ambientales → KPIs financieros → análisis combinado ESG-finanzas).

**Example Request (single-step):**
...
**Example Request (multi-step regulatory compliance):**
...
curl -X POST "http://localhost/api/chat/messages/" \
  ...
  -d '{
        "session": 5,
        "content": "Evalúa el nivel de cumplimiento CSRD de este cliente",
        "analysis_mode": "regulatory_compliance"
      }'
```
  ...
  "assistant_message": {
    ...
    "metadata": {
      ...
      "agent_steps": [
        {
          "key": "regulatory_requirements",
          "answer": "Lista de requisitos CSRD relevantes...",
          "chunk_ids": [45, 47],
          "usage": {
            "input_tokens": 60,
            "output_tokens": 40,
